		...
		module CountryAndRegionHelper
			...
			module InstanceMethods
				def country_select_tag(object_string, country_id = nil, region_id = nil)
					...
					html << "<script type=\"text/javascript\">if(Prototype){Element.observe(window, 'load', function(){new CountrySelect({country_select_id : '#{object_string}_country_id', region_select_id : '#{object_string}_region_id'});});}else{document.getElementById('#{object_string}_country_id').style.display = 'none';document.getElementById('#{object_string}_region_id').style.display = 'none';}</script>"
					html
				end
			end
